...
public class AddDoHeaderFooter implements IEventHandler{
	protected Document doc;
	private Customer customer;
	private Invoice invoice;
	private AppTrn appTrn;
	private String doDisplayNo;
	private String appName;
	Font boldFont;
	Font normalFont;
	boolean addHeader;
	public AddDoHeaderFooter(String appName,Document document,Customer customer,Invoice invoice,AppTrn appTrn, boolean addHeader, String doDisplayNo){
		this.doDisplayNo = doDisplayNo;
...
//    	}
    ...
    public Table getHeaderTable() {
    	...
    	Table doTable = new Table(new float[] {1,1}).useAllAvailableWidth();
    	doTable.addCell(new Cell().add(new Paragraph("Delivery Order : "+doDisplayNo).setFont(MearFonts.getFonts().TITLE_FONT).setFontSize(11)).setTextAlignment(TextAlignment.LEFT).setBorder(Border.NO_BORDER));
    	doTable.addCell(new Cell().add(new Paragraph("Quotation No : "+invoice.getQuatationNo().replace('-', '/')).setFont(MearFonts.getFonts().SUBTITLE_FONT).setFontSize(11)).setTextAlignment(TextAlignment.RIGHT).setBorder(Border.NO_BORDER));
    	headerTable.addCell(new Cell().add(new Paragraph().setFont((MearFonts.getFonts().TITLE_FONT)).setFontSize(16).setTextAlignment(TextAlignment.CENTER).add(new Text("DELIVERY ORDER").setUnderline())).setBorder(Border.NO_BORDER));
    	...
    	headerTable.addCell(new Cell().add(doTable).setBorder(Border.NO_BORDER));
    	return headerTable;
    }
