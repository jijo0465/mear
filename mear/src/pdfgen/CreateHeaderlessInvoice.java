...
public class CreateHeaderlessInvoice{
	Document document;
	PdfDocument pdfDoc;
	int tablePageNum;
	...
	int totalPages;
	Table table;
	public CreateHeaderlessInvoice(String dest,String appName, Customer customer,Invoice invoice, AppTrn appTrn) throws FileNotFoundException{
...
//		String dest=getServletConfig.getServletContext.getRealPath("/someFolder");
    	...
    	pdfDoc.addEventHandler(PdfDocumentEvent.START_PAGE, new AddHeaderFooter(appName,document,customer,invoice,appTrn,false));
//    	pdfDoc.addEventHandler(PdfDocumentEvent.END_PAGE, new AddFooter(document));
    ...
    public void addInvoiceTable(List<InvoiceItem> invoiceData) {
    	table = new Table(new float[]{40,290, 50, 40, 60, 70});
    	table.setWidth(550);
